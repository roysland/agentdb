...
- **`ChunkRepo.CreateReturningID` vs `Create`** — resolved. `Create` is used by the CLI `index` command, which doesn't need the row ID after inserting. `CreateReturningID` is used exclusively by the MCP server's `index_codebase` handler (`cmd/mcp.go:1120`, `1202`), which needs the row ID to associate chunks with their file records during streaming MCP indexing. The split is caller-driven, not a schema limitation.

- **`SourceFileRepo` vs `IndexedFileRepo`** — these serve the two independent pipelines. `SourceFileRepo` (`source_files` table) is the incremental manifest for the `analyze` pipeline: it stores AST metadata (language, package, LOC, symbol count) and is used to detect what files need re-parsing. `IndexedFileRepo` (`indexed_files` table) is the incremental manifest for the `index` pipeline: it stores chunk count and index status. A file can appear in one table and not the other if only one pipeline has been run, or if they were run at different times. They diverge intentionally — see `okf/data-flow.md`.
