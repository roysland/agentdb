...
- **`EmbeddingModel` on `Chunk`** — vestigial field. Embedding support was removed in commit `95703f8` (see `okf/decisions/drop-embedding-semantic-search.md`). The field is declared but never set or persisted anywhere in the current codebase. It can be ignored by all callers; treat it as always-empty string.
- **`parentSig` breadcrumb in treesitter chunking** — this parameter exists in the treesitter-specific chunker (behind the `treesitter` build tag, not in this module's non-treesitter source). Its behavior in the treesitter build: when a node is too large and is subdivided, sub-chunks carry a `"ParentName > ChildName"` breadcrumb signature so retrieval results can identify their nesting context. The redundancy with the first sub-chunk's own signature is intentional — the breadcrumb is for human readability of search results, not deduplication.
